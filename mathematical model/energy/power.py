...
def drag(boat: Boat, coeficientOfDrag: float = 1.09) -> float:
    return coeficientOfDrag * 1000 * boat.areaUnderWater() * boat.speed**2.0 * boat.alpha  # Use boat.alpha
...
def calculateWhPerDistance(boats: List[Boat], distance: float, powerUsage: float) -> float:
...
def calculateTotalEnergyKWh(boats: List[Boat], distance: float, powerUsage: float) -> float:
    timeToTravel = distance / boats[0].speed  # time in seconds
